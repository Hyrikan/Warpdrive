...
static void warpfdwBeginForeignScan(ForeignScanState *node, int eflags){
    ...
    if(ret != 0){
        elog_debug("Can't connect to warpserver! Code: %d", ret);
    }
    fetchedDataLength = 0;
    fetchedDataUsed = 0;
    fetchedDataBuf = NULL;
...
/*
 * Assumes at (fetchedDataBuf+fetchedDataUsed) the next length integer and returns it.
 */
int getNextValLength(){
    size_t spaceLeft = fetchedDataLength-fetchedDataUsed;
    int result;
    ...
    // is the whole value in our current buffer? just return it..
    if(spaceLeft >= 4){
        result = * (int *) (fetchedDataBuf+fetchedDataUsed);
        fetchedDataUsed += intsize;
        return result;
    }
    elog_debug("=============================  VALUE OVER BUFFER BOUNDARIES ============================");
    // else we need to put it together from this buffer and the next, we need to fetch in between.
    ...
    fetchedDataUsed = 0;
    // copy part two from new buffer
    memcpy((result+spaceLeft), (fetchedDataBuf+fetchedDataUsed), (intsize-spaceLeft));
    fetchedDataUsed += (intsize - spaceLeft);
    return result;
}

/*
 * Fetches one row and assumes row format in the received data buffer. SPECIALIZED FOR pg1_sfX_lineitem without decimals and dates! TODO handle numerics and dates
 */
static HeapTuple warpfdwFetchOneRowFromRowLineitem(TupleDesc tDesc){
    ...
    nextvalue = getNextBufferVal(nextValLength);
    elog_debug("Vallen: %d ends at index %ld", nextValLength, fetchedDataUsed);
    int32_t id2 = __bswap_32((*(int32_t *)nextvalue));
    ...
    nextvalue = getNextBufferVal(nextValLength);
    elog_debug("Vallen: %d ends at index %ld", nextValLength, fetchedDataUsed);
    int32_t id3 = __bswap_32((*(int32_t *)nextvalue));
    ...
    nextvalue = getNextBufferVal(nextValLength);
    elog_debug("Vallen: %d ends at index %ld", nextValLength, fetchedDataUsed);
    int32_t id4 = __bswap_32((*(int32_t *)nextvalue));
