...
#break cache to rebuild from here
ENV SOMERANDOMVARIABLE=lola

#my datasources
COPY datasources /etc/clickhouse-jdbc-bridge/config/datasources
...
# Get netcat for plain and fast file transfer
RUN apt-get update
...
# install cmake
RUN apt-get update && apt-get install -y gcc clang clang-tools cmake
...
# create build environemt with cmake
RUN mkdir /warpServer/build
WORKDIR /warpServer/build
RUN cmake ..

# compile target server
RUN cmake --build . --target click_warpserver_cpp
